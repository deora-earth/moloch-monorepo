...
contract("Moloch", accounts => {
  ...
  it("submit proposal", async () => {
    ...
    for (let x = 1; x <= applicants.length; x++) {
      ...
      let tx = await moloch.submitProposal(
        accounts[x],
        applicant.tokenTribute,
        applicant.sharesRequested,
        JSON.stringify({ title: `proposal number ${proposalIndex}`, description: `from: ${accounts[x]}` })
      );
